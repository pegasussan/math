function desenha(base,nNumeros) {
	...
	for (var i=0;i<nNumeros;i++) {
		var tx = raio * Math.sin(2 * Math.PI / nNumeros * i) + midX;
		var ty = raio * Math.cos(2 * Math.PI / nNumeros * i) + midY;
		coords.push([tx,ty]);
	}
	...
	function doThis(i) {
		setTimeout(function() {
			ctx.beginPath();
			ctx.arc(coords[i][0],coords[i][1],Math.sqrt(Math.pow(coords[i][0]-coords[0][0],2)+Math.pow(coords[i][1]-coords[0][1],2)),0,2*Math.PI);
			ctx.stroke();
			if (i == nNumeros - 1) {
